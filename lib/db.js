...
async function getFullState(groupId) {
  const [grp, members, cats, items, events, activity] = await Promise.all([
    ...
    pool.query('SELECT * FROM group_activity WHERE group_id=$1 ORDER BY created_at DESC LIMIT 100', [groupId]),
  ]);
