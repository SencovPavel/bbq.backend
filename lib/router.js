...
const {
  statsOverview, statsGroups, statsActivity, statsTechnical, statsGroupDetail,
  archiveAdminGroup,
}                                       = require('./controllers/admin');
const {
  listAdminUsers, getAdminUser, patchAdminUser,
}                                       = require('./controllers/admin-users');
const {
  listAdminEvents, getAdminEventDetail, statsFunnel, statsRetention,
}                                       = require('./controllers/admin-events');
const { adminIntegrations, adminHealth } = require('./controllers/admin-integrations');
const { exportGroupsCsv, exportAnalyticsCsv, listAuditLog } = require('./controllers/admin-export');
const {
  startYandex, yandexCallback,
  startApple, appleCallback,
  register, devLogin, login, logout, me, patchProfile,
...
const ROUTES = [
  ...
  { method: 'PATCH', path: '/auth/profile',           handler: patchProfile },

  { method: 'GET',  path: '/admin/stats/overview',    handler: statsOverview },
  { method: 'GET',  path: '/admin/stats/groups',      handler: statsGroups },
  { method: 'GET',  path: /^\/admin\/stats\/groups\/(?<groupId>[^/]+)\/events\/(?<eventId>[^/]+)$/, handler: getAdminEventDetail },
  { method: 'GET',  path: /^\/admin\/stats\/groups\/(?<groupId>[^/]+)$/, handler: statsGroupDetail },
  { method: 'POST', path: /^\/admin\/groups\/(?<groupId>[^/]+)\/archive$/, handler: archiveAdminGroup },
  { method: 'GET',  path: '/admin/stats/activity',    handler: statsActivity },
  { method: 'GET',  path: '/admin/stats/technical',   handler: statsTechnical },
  { method: 'GET',  path: '/admin/stats/funnel',      handler: statsFunnel },
  { method: 'GET',  path: '/admin/stats/retention',   handler: statsRetention },
  { method: 'GET',  path: '/admin/events',            handler: listAdminEvents },
  { method: 'GET',  path: '/admin/users',             handler: listAdminUsers },
  { method: 'GET',  path: /^\/admin\/users\/(?<id>[^/]+)$/, handler: getAdminUser },
  { method: 'PATCH', path: /^\/admin\/users\/(?<id>[^/]+)$/, handler: patchAdminUser },
  { method: 'GET',  path: '/admin/integrations',      handler: adminIntegrations },
  { method: 'GET',  path: '/admin/health',            handler: adminHealth },
  { method: 'GET',  path: '/admin/export/groups.csv', handler: exportGroupsCsv },
  { method: 'GET',  path: '/admin/export/analytics.csv', handler: exportAnalyticsCsv },
  { method: 'GET',  path: '/admin/audit-log',         handler: listAuditLog },
];
