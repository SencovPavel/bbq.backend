...
const { pool, DEFAULT_CATS, getFullState, readBody, readFormBody, BodyTooLargeError } = require('./db');
...
async function handleRequest(req, res, { broadcast, wss }) {
  ...
  if (req.method === 'POST' && url.pathname === '/tg-webhook') {
    const tgCheck = verifyWebhookSecret(req, {
      envVar: 'TG_WEBHOOK_SECRET',
      header: 'x-telegram-bot-api-secret-token',
      label: 'Telegram webhook',
    });
    if (!tgCheck.ok) {
      res.writeHead(tgCheck.statusCode, H);
      res.end(JSON.stringify({ error: tgCheck.reason }));
      return;
    }
  ...
  if (req.method === 'POST' && url.pathname === '/max-webhook') {
    const maxCheck = verifyWebhookSecret(req, {
    ...
    if (!maxCheck.ok) {
      res.writeHead(maxCheck.statusCode, H);
      res.end(JSON.stringify({ error: maxCheck.reason }));
      return;
    }
    const body = await readBodySafe(req, res, H);
    if (body === null) return;
    ...
    try {
      ...
      if (body.user) {
        try {
          ...
          // невалидный JSON в body.user — оставляем name по умолчанию
        }
      }
  ...
  if (req.method === 'POST' && url.pathname === '/auth/login') {
    ...
    const loginBody = await readBodySafe(req, res, H);
    if (loginBody === null) return;
