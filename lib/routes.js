...
async function handleRequest(req, res, { broadcast, wss }) {
  ...
  if (req.method === 'POST' && url.pathname === '/groups') {
    ...
    await pool.query('INSERT INTO group_members(group_id,user_id,name,is_admin) VALUES($1,$2,$3,TRUE)', [id, userId, userName]);
  ...
  if (req.method === 'POST' && url.pathname === '/groups/join') {
    ...
    await logActivity(pool, { groupId: group.id, type: 'member:joined', actorName: userName });
  ...
  if (req.method === 'POST' && url.pathname === '/auth/register') {
    ...
    if (password.length < 6) {
      res.writeHead(400, H); res.end(JSON.stringify({ error: 'Пароль минимум 6 символов' })); return;
    }
