...
const { pool: dbPool, getFullState: dbGetFullState, withTransaction } = require('./db');
...
async function seedEventTypeCategories(pool, groupId, eventType) {
  ...
  const missing = template.categories.filter(c => !existingTitles.has(c.title.trim().toLowerCase()));
  if (!missing.length) return;

  // Один multi-row INSERT вместо цикла запросов: ($1,$2,$3,$4,$5),($6,$7,...)…
  const params = [];
  const tuples = missing.map((cat) => {
    const base = params.length;
    params.push(nanoid(6), groupId, cat.title, cat.icon, nextPos++);
    return `($${base + 1},$${base + 2},$${base + 3},$${base + 4},$${base + 5})`;
  });
  await pool.query(
    `INSERT INTO categories(id,group_id,title,icon,position) VALUES${tuples.join(',')}`,
    params,
  );
}
...
/** Закрывает и убирает из комнаты все сокеты указанного пользователя (после kick/leave). */
function disconnectUserSockets(groupId, targetUserId) {
  const room = rooms.get(groupId);
  if (!room || !targetUserId) return;
  for (const sock of [...room]) {
    if (sock._userId === targetUserId) {
      room.delete(sock);
      try { sock.close(); } catch { /* сокет уже закрыт */ }
    }
  }
  if (!room.size) rooms.delete(groupId);
}
...
/**
 * isAdmin + отказ клиенту: при отсутствии прав шлёт error-фрейм `forbidden` в senderWs.
 * Возвращает true, если можно продолжать обработку сообщения.
 * @returns {Promise<boolean>}
 */
async function requireAdmin(groupId, userId, pool, senderWs = null) {
  if (await isAdmin(groupId, userId, pool)) return true;
...
async function dispatchMessage(msg, { groupId, userId, userName = null, platform = null, pool, getFullState, broadcast: bcast, senderWs = null }) {
  ...
  if (msg.type === 'item:add') {
    ...
    // Валидируем принадлежность до INSERT: нарушение FK по event_id иначе роняет обработчик,
    // а несуществующая категория делает позицию невидимой в UI (см. cat_id — без FK).
    if (msg.eventId) {
      const evt = await pool.query('SELECT id FROM events WHERE id=$1 AND group_id=$2', [msg.eventId, groupId]);
      if (!evt.rows.length) { if (senderWs) sendWsError(senderWs, 'event_not_found'); return; }
    }
    const catOk = await pool.query('SELECT id FROM categories WHERE id=$1 AND group_id=$2', [msg.catId, groupId]);
    if (!catOk.rows.length) { if (senderWs) sendWsError(senderWs, 'category_not_found'); return; }
    const id = nanoid(8);
    ...
    notifyGroupChat(pool, groupId, `➕ ${escapeHtml(userName || 'Кто-то')} добавил «${escapeHtml(msg.name)}»`);
  ...
  if (msg.type === 'item:update') {
    ...
    if (msg.field === 'bought' && msg.value === true) {
      ...
      if (boughtRow) {
        ...
        notifyGroupChat(pool, groupId, `✅ ${escapeHtml(userName || 'Кто-то')} купил «${escapeHtml(boughtRow.name)}»`);
      }
    } else {
      trackEvent(pool, {
        type: 'item:updated',
  ...
  if (msg.type === 'cat:add') {
    if (!await requireAdmin(groupId, userId, pool, senderWs)) return;
    const id  = nanoid(6);
  ...
  if (msg.type === 'cat:delete') {
    ...
    await withTransaction(pool, async (client) => {
      await client.query('DELETE FROM items WHERE cat_id=$1 AND group_id=$2', [msg.id, groupId]);
      await client.query('DELETE FROM categories WHERE id=$1 AND group_id=$2', [msg.id, groupId]);
    });
    bcast(groupId, { type: 'state', state: await getFullState(groupId) });
    return;
  }
  ...
  if (msg.type === 'event:add') {
    ...
    await withTransaction(pool, async (client) => {
      ...
      if (eventType) await seedEventTypeCategories(client, groupId, eventType);
    });
    await logActivity(pool, {
  ...
  if (msg.type === 'event:complete') {
    if (!await requireAdmin(groupId, userId, pool, senderWs)) return;
    const evtRes = await pool.query('SELECT name FROM events WHERE id=$1 AND group_id=$2', [msg.id, groupId]);
  ...
  // (family:group_settings WS-обработчик удалён — фронт использует HTTP POST /family/members/:id/groups)
  ...
  if (msg.type === 'member:remove') {
    ...
    bcast(groupId, { type: 'state', state: await getFullState(groupId) });
    disconnectUserSockets(groupId, msg.userId); // выкидываем ушедшего из комнаты
    return;
  }
  ...
  if (msg.type === 'member:leave') {
    ...
    disconnectUserSockets(groupId, userId);
    return;
  }
...
function createWsHandler(wss) {
  wss.on('connection', (ws, req) => {
    ...
    // Пер-соединение rate-limit: не больше WS_MSG_LIMIT сообщений в скользящем окне WS_MSG_WINDOW_MS.
    ...
    ws.on('message', async (raw) => {
      ...
      const now = Date.now();
      if (now - winStart > WS_MSG_WINDOW_MS) { winStart = now; winCount = 0; }
      if (++winCount > WS_MSG_LIMIT) { sendWsError(ws, 'rate_limited'); return; }
      ...
      if (msg.type === 'join') {
        ...
        ws._userId = userId; // нужно для адресного отключения при kick/leave
      ...
      await dispatchMessage(msg, {
        ...
        console.error('WS message handler:', e.message);
        sendWsError(ws, 'internal_error');
      }
    });
