 ...
 */
async function dispatchMessage(msg, { groupId, userId, userName = null, pool, getFullState, broadcast: bcast, senderWs = null }) {
  ...
  if (msg.type === 'item:add') {
    ...
    trackEvent(pool, { type: 'item:added', userId, groupId });
  ...
  if (msg.type === 'event:add') {
    ...
    trackEvent(pool, { type: 'event:created', userId, groupId });
    bcast(groupId, { type: 'state', state: await getFullState(groupId) });
    return;
  }
  ...
  if (msg.type === 'family:rsvp') {
    ...
    trackEvent(pool, { type: 'rsvp:set', userId, groupId, meta: { attending: msg.attending } });
    bcast(groupId, { type: 'state', state: await getFullState(groupId) });
    return;
  }
