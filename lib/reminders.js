  ...
  for (const row of rows) {
    ...
    try {
      if (daysLeft > 1 && daysLeft <= 4 && !row.reminder_4d_sent) {
        await sendMessage(row.tg_chat_id, `⏳ До «${row.name}» осталось ${daysLeft} ${pluralDays(daysLeft)} — не забудьте собрать список`);
        ...
        await sendMessage(row.tg_chat_id, `🔥 «${row.name}» уже ${when}!`);
