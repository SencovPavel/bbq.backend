...
async function register(req, res, { H }) {
  ...
  trackEvent(pool, { type: 'user:registered', userId: id, platform: detectPlatform(req) });
  res.writeHead(200, H);
  res.end(JSON.stringify({
    id, name: name.trim(), email: normalEmail,
    bio: null,
...
async function devLogin(req, res, { H }) {
  ...
  let { rows } = await pool.query(
    'SELECT id, name, email, is_superadmin FROM users WHERE email=$1',
    [email.toLowerCase()],
  );
  if (!rows.length) {
    const id = nanoid(10);
    await pool.query(
      'INSERT INTO users(id, email, password_hash, name, is_superadmin) VALUES($1,$2,$3,$4,TRUE)',
      [id, email.toLowerCase(), hash, name],
    );
    rows = [{ id, name, email: email.toLowerCase(), is_superadmin: true }];
  } else {
    // Синхронизируем пароль, имя и права суперадмина с .env
    await pool.query(
      'UPDATE users SET password_hash=$1, name=$2, is_superadmin=TRUE WHERE id=$3',
      [hash, name, rows[0].id],
    );
    rows[0].is_superadmin = true;
  }
  await createSession(res, rows[0].id);
  res.writeHead(200, H);
  res.end(JSON.stringify({
    id: rows[0].id, name, email: email.toLowerCase(),
    bio: null,
...
async function login(req, res, { H }) {
  ...
  // Dev-аккаунт из .env всегда суперадмин (локально и при входе в admin)
  const devEmail = process.env.DEV_USER_EMAIL?.toLowerCase();
  if (process.env.NODE_ENV !== 'production' && devEmail && user.email === devEmail) {
    await pool.query('UPDATE users SET is_superadmin=TRUE WHERE id=$1', [user.id]);
    user.is_superadmin = true;
  }
  await createSession(res, user.id);
  trackEvent(pool, { type: 'user:login', userId: user.id, platform: detectPlatform(req) });
  res.writeHead(200, H);
  res.end(JSON.stringify({
...
async function patchProfile(req, res, { H }) {
  ...
  if (bio !== undefined && typeof bio !== 'string') {
    res.writeHead(400, H); res.end(JSON.stringify({ error: 'bio must be a string' })); return;
  }
  const { rows } = await pool.query(
    `UPDATE users SET bio = COALESCE($1, bio) WHERE id = $2 RETURNING id, name, email, bio`,
    [bio?.slice(0, 500) ?? null, user.id],
  );
  res.writeHead(200, H);
...
module.exports = { startYandex, yandexCallback, startApple, appleCallback, register, devLogin, login, logout, me, patchProfile };
