...
async function statsOverview(req, res, { H }) {
  ...
  const [
    groups, registeredUsers, webUsers, telegramUsers, groupParticipants,
    membersWithoutAccount, activeEvents, items,
    ...
    pool.query(`
      ...
      WHERE email NOT LIKE 'tg_%@telegram.internal'
    `),
    pool.query(`
      SELECT COUNT(*)::int AS n FROM users
      WHERE email LIKE 'tg_%@telegram.internal'
    `),
  ...
  res.end(JSON.stringify({
    ...
    webUsers: webUsers.rows[0].n,
    telegramUsers: telegramUsers.rows[0].n,
    groupParticipants: groupParticipants.rows[0].n,
    membersWithoutAccount: membersWithoutAccount.rows[0].n,
