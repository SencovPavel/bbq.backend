...
async function handleUpdate(update, pool, broadcast) {
  ...
  if (update.my_chat_member) {
    ...
    if (wasOut && isIn && chat.type !== 'private') {
      // Создаём группу если ещё не привязана
      const existing = await pool.query(
        'SELECT id FROM picnic_groups WHERE tg_chat_id=$1', [chat.id]
      );
      if (!existing.rows.length) {
        const groupId    = nanoid(10);
        const inviteCode = nanoid(6).toUpperCase();
        const groupName  = chat.title || 'Пикник';
        await pool.query(
          'INSERT INTO picnic_groups(id,invite_code,name,tg_chat_id) VALUES($1,$2,$3,$4)',
          [groupId, inviteCode, groupName, chat.id]
        );
        const appUrl = process.env.PUBLIC_URL || 'https://bbq.teaspv.pro';
        await sendMessage(chat.id,
          `🔥 <b>Группа «${groupName}» создана!</b>\n\n` +
          `Открывайте приложение и присоединяйтесь по коду:\n` +
          `<b>${inviteCode}</b>\n\n` +
          `🔗 ${appUrl}\n\n` +
...
async function getFullState(groupId, pool) {
  ...
  return { group: grp.rows[0], members: members.rows, categories: cats.rows, items: items.rows, events: events.rows };
}
...
module.exports = { handleUpdate, setWebhook, sendMessage, resolveCatId, isDuplicateName };
